...
from olmo.dummy_quant import fake_tensor_quantize, fake_tensor_dequantize
...
class A8Linear(torch.autograd.Function):
    ...
    def forward(ctx, x, weight, bias):
        ...
        if x.dtype == torch.half:
            ...
            x_dtype = 'fp32'
        # print('activation',x_dtype,'weight',weight.dtype)
    ...
    def backward(ctx, grad_output):
        ...
        # Compute gradients
        # print("grad dtype",grad_output.dtype,"weight dtype",weight.dtype)
        if grad_output.dtype==torch.float:
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True,
        ...
        if weight_data is not None:
            if weight_data.device.type == "meta":
                # keep everything meta: just swap the parameter
                self.register_parameter("weight", nn.Parameter(weight_data,requires_grad=self.weight.requires_grad))
            else:
                self.weight.data.copy_(weight_data)
        if bias_data is not None and bias is not None:
            if weight_data.device.type == "meta":
                # keep everything meta: just swap the parameter
                self.register_parameter("bias", nn.Parameter(bias_data,requires_grad=self.bias.requires_grad))
            else:
                self.bias.data.copy_(bias_data)
        # print(weight_data.dtype,self.weight.data.dtype)
...
def prepare_model_for_int8_training_simulation_act_weight(model, target_module):

    for name, module in reversed(model._modules.items()):

        if len(list(module.children())) > 0:
            model._modules[name] = prepare_model_for_int8_training_simulation_act_weight(module, target_module)
        ...
        if isinstance(module, nn.Linear):
            ...
            new_layers = QLinear(in_features, out_features, bias=bias, device=weight_data.device, 
                weight_data=weight_data, bias_data=bias_data, 
                num_bits=4, group_size=256, stochastic_round=True)
